...
const Asset = require('../../../models/finances/accounting/Asset')
 ...
 *                  -   Accounting
 *                      -   Asset
 *          description: Use to to request all products. Model Asset
 *          responses:
 *              '200':
...
{
    try {
        const products = await Asset.find().lean().exec()

        response.status(200).send({
            "success":"true",
            "message": "Assets have been retrieved succesfully",
            "data": products
        })
    } catch (error) {
...
async function show(request, response)
{
    ...
    if( foundProduct != undefined)
    {
        response.send({
            "success":"true",
            "message": `Asset with id ${id} has been retrieved succesfully`,
            "data": foundProduct
        });
    }
}
...
async function store(request, response)
{
    ...
    const newProduct = Asset(request.body)

    newProduct.save()

    response.status(200).send({
        "success":"true",
        "message": `The new asset has been saved succesfully`,
        "data": newProduct
    })
}
...
async function update (request, response)
{
   ...
   */
    ...
    if ( foundProduct != undefined )
    {
        ...
        response.status(200).send({
            "success":"true",
            "message": `Asset with id ${id} has been updated succesfully`,
            "data": foundProduct
        });
    }
...
async function destroy (request, response)
{
    ...
    //if ( foundProduct != undefined )
    {
        const result = await Asset.remove({sid: id})

        response.status(200).send({
            "success":"true",
            "message": `Asset with id ${id} has been deleted succesfully`,
            "data": result
        });
    }
...
{
    let foundProduct = await Asset.findOne({sid: id})
